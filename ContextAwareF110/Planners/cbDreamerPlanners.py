import numpy as np 
from ContextAwareF110.Utils.cbdreamer.dreamerv3 import DreamerV3
from ContextAwareF110.Utils.HistoryStructs import TrainHistory
from ContextAwareF110.Utils.FastTransform import FastTransform
import torch
from numba import njit

from ContextAwareF110.Utils.utils import init_file_struct
...
class cbDreamerTester:
    ...
    def plan(self, obs, context=None, **kwargs):
        ...
        if obs['state'][3] < self.v_min_plan:
            ...
            return {
                'action': self.action,
                'reconstruction': np.zeros((1, 2), dtype=np.float32)
            }   
        ...
        return {
            'action': self.action,
            'reconstruction': recon.cpu().numpy() # TODO fix this later
        }   
